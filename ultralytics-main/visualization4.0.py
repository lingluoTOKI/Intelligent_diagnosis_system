    ...
    def _receive_loop(self):
        ...
        while self.is_running:
            try:
                ...
                frame_buffers[frame_seq][packet_idx] = payload
                
                # 检查是否接收完整
                if len(frame_buffers[frame_seq]) == total_packets:
                    # 拼接帧数据
                    frame_data = b''.join([frame_buffers[frame_seq][i] for i in range(total_packets)])
                    ...
                    # 清理缓存
                    del frame_buffers[frame_seq]
                    
            except socket.timeout:
                continue
            except Exception as e:
                if self.is_running:
                    print("摄像头接收错误: {}".format(e))
                break
...
4. 确保网络连接正常
        """
        self.show_message_box("摄像头启动", instructions, QMessageBox.Information)
