...
class SystemLauncher:
    ...
    def check_dependencies(self):
        ...
        required_files = [
            ...
            "src/pc/pc_diagnosis_server.py",
            "src/pc/pc_voice_server.py",
            "src/board/board_camera_integration.py",
            "src/utils/latency_optimizer.py",
            "src/network/wifi_pc_sender_with_mouse.py",
            "src/network/wifi_pc_receiver_2.0.py"
        ]
    ...
    def start_pc_services(self):
        ...
        if self.config["services"]["diagnosis_server"]:
            services.append({
                "name": "诊断服务器",
                "command": f"{sys.executable} src/pc/pc_diagnosis_server.py",
                "delay": 3
            })
        ...
        if self.config["services"].get("voice_server", True):
            services.append({
                "name": "语音服务器",
                "command": f"{sys.executable} src/pc/pc_voice_server.py",
                "delay": 4
            })
        ...
        if self.config["services"]["screen_sharing"]:
            services.append({
                "name": "屏幕共享",
                "command": f"{sys.executable} src/network/wifi_pc_sender_with_mouse.py",
                "delay": 6
            })
    ...
    def generate_board_commands(self):
        ...
        if self.config["services"]["screen_sharing"]:
            print("1. 启动屏幕接收服务:")
            print(f"   python src/network/wifi_pc_receiver_2.0.py")
            print()
        
        if self.config["services"]["camera_integration"]:
            print("2. 启动摄像头诊断服务:")
            print(f"   python src/board/board_camera_integration.py")
            print("   在菜单中选择 'p' 进入预览模式")
            print()
            
        print("3. 启动语音对话服务:")
        print(f"   python src/board/board_voice_interaction.py")
        print("   或在摄像头预览中按 'r' 键")
        print()
...
# 启动屏幕接收 (后台)
if [ -f "src/network/wifi_pc_receiver_2.0.py" ]; then
...
# 启动摄像头诊断服务
if [ -f "src/board/board_camera_integration.py" ]; then
