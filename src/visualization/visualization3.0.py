    ...
    def _init_vosk(self):
        ...
        try:
            ...
            if candidates:
                ...
                print(f"[DEBUG] Vosk模型加载成功: {self.vosk_model_dir}")
            else:
                self.vosk_model = None
                self._vosk_json = None
            ...
            self.vosk_model = None
            self._vosk_json = None
            print(f"[DEBUG] Vosk不可用: {e}")  # 不是错误，只是提示
    ...
    def _perform_recognition(self):
        ...
        try:
            ...
            with sr.Microphone() as source:
                ...
                # 用户自定义录音时长，无时间限制
                audio = self.local_recognizer.listen(source, timeout=30, phrase_time_limit=None)
    ...
    def get_custom_advice(self, prompt):
        ...
        for attempt in range(max_retries):
            try:
                ...
                print(f"尝试连接DeepSeek API (第{attempt + 1}次)...")
                ...
                if response.status_code == 200:
                    ...
                    if "choices" in result and len(result["choices"]) > 0:
                        ...
                        print(f"API响应格式异常: {result}")
                        return "抱歉，AI回复格式异常，请稍后重试。"
                elif response.status_code == 401:
                    return "API密钥无效，请检查您的DeepSeek API密钥是否正确。"
                elif response.status_code == 429:
                    return "API调用频率过高，请稍后再试。"
                elif response.status_code == 500:
                    if attempt < max_retries - 1:
                        print(f"服务器错误，{2}秒后重试...")
                        import time
                        time.sleep(2)
                        continue
                    ...
                    print(f"API请求失败: {response.status_code} - {response.text}")
                    return f"API请求失败，错误代码: {response.status_code}\n\n错误详情: {response.text[:200]}"
                ...
                if attempt < max_retries - 1:
                    print(f"网络连接错误，{3}秒后重试...")
                    import time
                    time.sleep(3)
                    continue
                return f"网络连接错误，请检查网络连接。\n\n**可能原因:**\n- 网络不稳定\n- 防火墙阻止连接\n- DNS解析问题\n- 需要代理访问\n\n**解决建议:**\n- 检查网络连接\n- 尝试使用VPN\n- 检查防火墙设置\n- 稍后重试\n\n**错误详情:** {str(e)}"
            except requests.exceptions.SSLError:
                return "SSL证书验证失败，请检查网络安全设置或尝试使用VPN。"
            except requests.exceptions.RequestException as e:
                if attempt < max_retries - 1:
                    print(f"网络请求错误，{2}秒后重试...")
                    import time
                    time.sleep(2)
                    continue
                return f"网络请求错误: {str(e)}\n\n**建议:** 请检查网络连接或稍后重试。"
            except Exception as e:
                print(f"获取自定义建议时出错: {e}")
                return f"获取建议时出错: {str(e)}\n\n**建议:** 请稍后重试或联系技术支持。"
        
        return "经过多次重试仍无法连接到DeepSeek API，请稍后再试。"
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # 延迟初始化语音组件
        QTimer.singleShot(500, self.init_speech_components)
    ...
    def init_ui(self):
        ...
        voice_layout.addWidget(self.voice_chat_enabled)
        voice_layout.addWidget(self.voice_input_button)
        voice_layout.addWidget(self.mic_test_button)
        ...
        right_layout.addWidget(advice_group, 6)  # 减少建议组的比例
        right_layout.addWidget(api_group, 2)     # API组
        right_layout.addWidget(chat_group, 2)    # 对话组
        ...
        if self.voice_manager is None:
            self.voice_manager = SmartVoiceManager(parent=self)
    ...
    def show_unified_response(self, question, answer):
        ...
        question_label.setStyleSheet(f"padding: 10px; font-size: 12px;")
    ...
    def show_ai_advice(self):
        ...
        try:
            # 检查是否启用API
            if self.use_api_checkbox.isChecked() and self.deepseek_api.api_key:
                # 使用API获取建议
...
"""
    ...
    def process_ai_response(self):
        ...
        try:
            ...
            # 更新进度
            QApplication.postEvent(self, AIResponseEvent("progress", 20, "连接AI服务..."))
            
            api_key = self.api_key_input.text().strip()
            if not api_key:
                # 使用默认建议
                QApplication.postEvent(self, AIResponseEvent("progress", 60, "生成默认建议..."))
                response = """# 🩺 AI医疗咨询建议
...
*💊 健康提示：早发现、早治疗是眼部疾病防治的关键*"""
                QApplication.postEvent(self, AIResponseEvent("progress", 100, "完成"))
                QApplication.postEvent(self, AIResponseEvent("completed", response))
                return
            
            # 使用AI API
            QApplication.postEvent(self, AIResponseEvent("progress", 40, "发送请求到AI..."))
            
            ai_service = MedicalAIService(api_key)
            response = ai_service.get_custom_advice(message)
            
            QApplication.postEvent(self, AIResponseEvent("progress", 80, "处理AI回复..."))
